...
# matches = LIKE
class SportsTag < ActiveRecord::Base
    ...
    keyword_arrays[f] = k.join
     ...
     where_clause = "(tag) GLOB ?"
      ...
      if keyword_arrays.length != 1  then
         ...
         where_clause_or = "'sports_tags'.'tag' GLOB ? OR 'sports_tags'.'tag' GLOB ?"
         where_clause_and = "SELECT id FROM sports_tags WHERE 'sports_tags'.'tag' GLOB ? INTERSECT SELECT id FROM 'sports_tags' WHERE 'sports_tags'.'tag' GLOB ?"

  
      search_A =  SportsTag.where([where_clause, "#{keyword_arrays[0]}"]).count 
      i = 0

           for f in 1..(SportsWord.count) do
            search_word_utf8 = SportsWord.where(:id => "#{f}").pluck(:name).join 
            search_word = NKF.nkf('-w16xm0', search_word_utf8).inspect.delete("\"").downcase 
            search_A_and_B = SportsTag.find_by_sql([where_clause_and, "#{keyword_arrays[0]}","#{search_word}"]).count
             if search_A_and_B > (search_A*0.001) then
              search_B = SportsTag.where([where_clause, "#{search_word}"]).count
